...
class MotorSimNode(Node):
    ...
    def __init__(self):
        ...
        # Parameters (tune later if you want)
        self.max_accel = 3.0     # "m/s^2" equivalent for full throttle
        self.max_brake = 3.0     # "m/s^2" equivalent for full brake
        self.vel_time_const = 0.8    # s, first-order dynamics
        self.brake_time_const = 0.3  # s, how fast brake position moves

        # State
        self.last_cmd_throttle = 0.0  # 0..1
        self.last_cmd_brake = 0.0     # 0..1
        self.velocity = 0.0           # m/s (simulated)
        self.brake_pos = 0.0          # 0..1 (0 = release, 1 = full)
        ...
        # Subscriber: same topic & type as embedded code
        self.cmd_sub = self.create_subscription(
            AckermannDrive,
            'driveData',           # use 'driveData' or '/driveData' to match your system
            self.cmd_callback,
            10
        )

        # Timer for simulation update (50 Hz)
        self.dt = 0.02
